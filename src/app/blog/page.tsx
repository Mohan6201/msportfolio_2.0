...
export default function BlogPage() {
  ...
  return (
    <main className="min-h-screen bg-darkBrown grid-bg">
      {/* Header */}
      <div className="relative pt-24 pb-16 px-4 border-b border-white/5">
        <div className="absolute -top-20 right-20 w-80 h-80 rounded-full bg-cyan/4 blur-[100px] pointer-events-none" />
        <div className="max-w-4xl mx-auto">
          <Link href="/" className="inline-flex items-center gap-2 text-lightGrey hover:text-cyan transition-colors text-sm font-mono mb-8">
            <ArrowLeft className="w-4 h-4" /> Back to Portfolio
          </Link>
          <div className="flex items-start gap-4 mb-4">
            <div className="w-12 h-12 rounded-xl bg-cyan/10 border border-cyan/20 flex items-center justify-center flex-shrink-0 mt-1">
              <Terminal className="w-5 h-5 text-cyan" />
            </div>
            <div>
              <p className="section-tag mb-2">Technical Writing</p>
              <h1 className="font-special text-4xl sm:text-5xl font-bold text-white">DevOps Blog</h1>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 mt-4 text-sm font-mono text-lightGrey/50">
        ...
        <div className="flex flex-col gap-5">
          {posts.map((post) => (
            <Link key={post.slug} href={`/blog/${post.slug}`}
              className="glass glass-hover rounded-2xl p-6 border border-white/5 group block">
              <div className="flex flex-wrap gap-2 mb-3">
                {post.tags.map((tag) => (
                  <span key={tag} className="badge badge-cyan">{tag}</span>
                ))}
              </div>
              <h2 className="font-special text-xl font-bold text-white group-hover:text-cyan transition-colors duration-200 mb-2 leading-snug">
                {post.title}
              </h2>
