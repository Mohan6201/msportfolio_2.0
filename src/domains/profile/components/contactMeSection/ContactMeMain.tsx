...
export default function ContactMeMain({ profile, socialLinks }: ContactMeMainProps) {
  const INFO = [
    ...
    { Icon: IoLocationOutline, label: "Location", value: profile.location, href: "#" },
  ];

  const SOCIALS = [
    { Icon: FiGithub,     href: profile.githubUrl ?? "#",   label: "GitHub",   color: "text-white hover:text-cyan" },
    { Icon: FiLinkedin,   href: profile.linkedinUrl ?? "#", label: "LinkedIn", color: "text-white hover:text-cyan" },
    { Icon: HiOutlineMail, href: `mailto:${profile.email}`, label: "Email",    color: "text-white hover:text-orange" },
  ];
